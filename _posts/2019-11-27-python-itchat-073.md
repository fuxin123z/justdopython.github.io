...
今天来为大家介绍一个有意思的开源微信个人号 API，它是基于 Python 调用微信网页版实现，可以只需要编写少量的代码，就可以完成一个能够处理所有信息的微信机器人。那它到底能实现了什么功能，接下来为大家一一介绍。
...
然后通过执行命令 `python mychat1.py` 运行代码，这时会生成一个名为 `QR.png` 的二维码图片文件，通过扫描这个二维码就可以登陆微信了，这时将会显示如下信息：
...
如果你向这个登陆后的微信发送文本消息，则会直接把你发送的消息内容打印出来。
...
通过以上几种可以看出，几种方法各有特点，可以随自己喜好使用，不过官方推荐使用 `send` 方法。
...
本文为大家简单介绍了 itchat 所提供的大部分功能，通过这些功能我们完全可以非常方便的开发出一个微信聊天机器人。另外 GitHub 上也有很多基于 itchat 做的开源机器人项目，有兴趣可以去搜索，再次感谢各位开源作者的贡献。
